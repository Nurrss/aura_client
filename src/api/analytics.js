...
export const getRoadmapPrediction = async (roadmapId) => {
  const response = await api.get(`/analytics/predict/${roadmapId}`);
  return response.data.data;
};
...
export const getWeeklyCoaching = async () => {
  const response = await api.post('/analytics/coaching/weekly');
  return response.data.data;
};

export const getGoalRecommendations = async () => {
  const response = await api.get('/analytics/recommendations/goals');
  return response.data.data;
};

export const getMilestoneSuggestions = async (goalId) => {
  const response = await api.post(`/analytics/suggest/milestones/${goalId}`);
  return response.data.data;
};
